...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.cap = "Ejecución del código sin `%>%`"}
library(DiagrammeR)
mermaid("
graph LR
  ...
  2(2) --> sqrt
  sqrt --> temp_variable[tmp_variable=1,41]
  temp_variable --> trunc
  trunc --> 1(1)
 
  class 2,1 style_main_node;
  class temp_variable style_temp_node;
  class sqrt,trunc style_func_node;
", height = '100%', width = '100%')
```


```{r echo = FALSE, warning = FALSE, message = FALSE, fig.cap = "Ejecución del código con `%>%`"}
library(DiagrammeR)
mermaid("
graph LR
  %% Set edges to be curved (try monotoneX for a nice alternative)
  ...
  class A,D style_main_node;
  class B,C style_sub_node;
", height = '100%', width = '100%')
```
